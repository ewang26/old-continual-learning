...
class ContinualLearningManager(ABC):
    ...
    def evaluate_task(
        ...
        for batch_x, batch_y in test_dataloader:
            ...
            # TO-DO: use "correct" variable here to simplify code
            total_correct += (
    ...
    def train(
        ...
        train_dataloader_list, test_dataloader, _, _ = self._get_task_dataloaders(use_memory_set, batch_size)
        # current_labels: List[int] = list(self._get_current_labels())
        current_labels = self._get_task_label_list()
        # Train on batches
        criterion_list = [nn.CrossEntropyLoss() for i in range(len(train_dataloader_list))] # CrossEntropyLoss for classification tasks
        optimizer = Adam(self.model.parameters(), lr=lr)
    ...
    def _get_task_dataloaders_0307(
        ...
        # combining all the x data in the memory sets for the memory tasks
    ...
    def _get_task_dataloaders(
        ...
        for i in range(len(combined_train_x) - 1):
            ...
            repeated_train_x.append(torch.from_numpy(np.tile(combined_train_x[i], memory_set_ratio)[:terminal_task_data_len]))
        ...
        # train_dataset = TensorDataset(combined_train_x, combined_train_y)
        # memory_test_dataset = TensorDataset(memory_test_x, memory_test_y)
    ...
    def _get_task_label_list(self):
        ...
        running_tasks = self.tasks[: self.task_index + 1]
        for task in running_tasks:
            label_list.append(list(task.task_labels))
        return label_list
